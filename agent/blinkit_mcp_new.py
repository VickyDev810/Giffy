...
# Configuration
API_BASE_URL = os.getenv('BLINKIT_API_URL', '')
...
async def list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            description="Verify OTP received on phone to complete login (automatically saves session)",
            inputSchema={
                "type": "object",
                "properties": {
                    "otp": {
        ...
        Tool(
            name="blinkit_save_session",
            description="Manually save current session (cookies and localStorage) for persistence",
            inputSchema={
                "type": "object",
                "properties": {},
            },
        ...
        # Complete Checkout (All-in-One)
        Tool(
            name="blinkit_complete_checkout",
            description="Complete entire checkout process in one call: checkout cart, select first address (index 0), select second UPI ID, and initiate payment. This is the fastest way to complete an order.",
            inputSchema={
                "type": "object",
                "properties": {},
            },
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    elif name == "blinkit_save_session":
        await track_usage("save_session", 0.001)
        result = await api_call("POST", "/auth/save-session")
        return [TextContent(type="text", text=json.dumps(result, indent=2))]
    ...
    # Complete Checkout (All-in-One)
    elif name == "blinkit_complete_checkout":
        await track_usage("complete_checkout", 0.10)
        result = await api_call("POST", "/checkout/complete")
        return [TextContent(type="text", text=json.dumps(result, indent=2))]
